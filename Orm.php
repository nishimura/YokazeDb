...
class YokazeDb_Orm
{
     ...
     * @access public
     */
    public function __construct(YokazeDb_Driver $db, $tableName = null){
        $this->dao = $db;
        if ($tableName)
    ...
    private function _createVo($tableName){
        ...
        $className = 'YokazeDb_Vo_'. $voClassSuffix;
    ...
    function save($vo, $where = null, $isForceInsert = false){
        ...
        if (!($vo instanceof YokazeDb_Vo)){
            trigger_error("$vo is not Vo object.", E_USER_WARNING);
            return false;
        }
     ...
     *
     * @param string $str
     * @param int $type e.g. PDO::PARAM_STR
     */
